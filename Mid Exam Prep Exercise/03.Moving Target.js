...
function movingTarget(input) {
    ...
    while (input.length > 0) {
        ...
        switch (curCommand) {
            ...
            case "Strike":
                let indexOfStrike = Number(command.split(" ")[1]);
                let radius = Number(command.split(" ")[2]);
                if (indexOfStrike >= 0 && indexOfStrike < sequenceOfTargets.length) {
                    let startIndex = Math.max(indexOfStrike - radius, 0);
                    let endIndex = Math.min(indexOfStrike + radius, sequenceOfTargets.length - 1);
                    let countToRemove = endIndex - startIndex + 1;

                    sequenceOfTargets.splice(startIndex, countToRemove);
                } else {
